		...
		*********************************************************************************************************/
		...
		char    GcRcvBuf2[20],GcRcvBuf3[20];                                                   /*  AD采集到的数据              */
		...
		*********************************************************************************************************/
		...
		// IOCON_PIO1_0 |= 0x02;
		IOCON_PIO1_1 &= ~0xBF;
		IOCON_PIO1_1 |= 0x02;				//PIO1_1模拟输入通道2
		IOCON_PIO1_2 &= ~0xBF;
		IOCON_PIO1_2 |= 0x02;				//PIO1_2模拟输入通道3

		AD0CR = ( 0x0C << 0 ) |                                             /*  SEL=12,选择ADC2，3（1100）               */
		...
		*********************************************************************************************************/
		...
		while (1) {
			...
			// for(i = 0; i < 10; i++) {
				...
				ulADCBuf2 = AD0DR2;                                          /*  读取结果寄存器              */
				ulADCBuf2 = (ulADCBuf2 >> 6) & 0x3ff;
				myDelay(50);
				// ulADCData1 += ulADCBuf1;
				while((AD0DR3 & 0x80000000) == 0){                          /*  读取AD0DR1的Done            */
				}
				ulADCBuf3 = AD0DR3;                                          /*  读取结果寄存器              */
				ulADCBuf3 = (ulADCBuf3 >> 6) & 0x3ff;
				// ulADCData2 += ulADCBuf2;
			// }
			// ulADCData1 = ulADCBuf1 / 10;                                     /*  采样10次进行虑波处理        */
			...
			sprintf(GcRcvBuf3,"VIN3 = %4d mv",ulADCData3);/*  将数据发送到串口进行显示    */
